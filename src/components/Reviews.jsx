      ...
      <div>
        <Reveal className="flex flex-col gap-4">
          <div className="flex items-center gap-4">
            <span className="font-mono text-xs font-bold text-copper-400">03</span>
          ...
          <h2 className="font-display text-[2.6rem] uppercase leading-[0.95] text-bone-50 sm:text-6xl">
            La barra habla, <span className="text-copper-300">no nosotros.</span>
          </h2>
        </Reveal>
      </div>
      ...
      <div className="mt-14 grid gap-6 md:grid-cols-3">
        {reviews.map((review, i) => (
          <Reveal key={review.name} delay={i * 0.08}>
            <figure className="group relative flex h-full flex-col justify-between gap-6 border-2 border-bone-50/10 bg-ink-900/50 p-7 transition-colors hover:border-copper-400/50">
              <span className="absolute -left-2 -top-2 bg-copper-400 px-2 py-0.5 font-mono text-xs font-bold text-ink-950">
                Nº 0{i + 1}
              </span>
              <div>
              ...
              <figcaption className="border-t-2 border-dashed border-bone-50/15 pt-4">
                ...
                <span className="mt-1 block font-mono text-[0.62rem] uppercase tracking-[0.22em] text-steel-400">
                  {review.role}
                </span>
              </figcaption>
            </figure>
